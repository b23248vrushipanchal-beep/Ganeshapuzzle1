// ============================================================================
// PUZZLE REVEAL CARD - Framer Code Component (ready to paste)
// ----------------------------------------------------------------------------
// HOW TO USE
//   1. In Framer: Insert > Code File > New File, paste this whole file in.
//   2. Drag the component onto your canvas - works at any frame size,
//      including 1080 x 1920 phone frames (it flips to a 3 x 4 portrait cut).
//   3. Set the Image in the property panel. Fonts need NO setup: the
//      component uses Inter - Framer's built-in default - with a clean
//      system fallback, so it resolves out of the box anywhere.
//
// BEHAVIOUR
//   Solve the 12-piece jigsaw -> confetti -> solved success animation.
//   Horizontal transition: the artwork slides completely off-screen to the right
//   while the white description card enters from the left and anchors in the center.
//   Tap the card or artwork to toggle. All motion is spring-driven via framer-motion.
// ============================================================================
...
const defaultInfoItems: InfoItem[] = [
    {
        title: "Materials",
        text: "Printed on archival matte stock, framed and ready to hang.",
    },
    {
        title: "Difficulty",
        text: "A relaxed 12-piece build - great for a quick, satisfying solve.",
    },
    {
        title: "Estimated Time",
        text: "Most people finish in under two minutes.",
    },
]
...
export default function PuzzleGame(props: PuzzleGameProps) {
    ...
    const isPortraitContainer = size.height > size.width
    const columns = isPortraitContainer ? 3 : 4
    const rows = isPortraitContainer ? 4 : 3
    ...
    }, [pieceBlueprint, size.height, size.width])
    ...
    useEffect(() => {
        ...
        const onPointerUp = (event: PointerEvent) => {
            ...
            setPieces((current) => {
                ...
                if (uniqueClusterCount === 1 && attach) {
...
addPropertyControls(PuzzleGame, {
    ...
    infoPanelHeight: {
        type: ControlType.Number,
        title: "Info Panel Height",
        description:
            "Height of the info panel that opens after solving the puzzle.",
        defaultValue: 500,
        min: 240,
        max: 800,
        step: 4,
